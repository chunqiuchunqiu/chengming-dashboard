...
import { financeErrorResponse, FinanceHttpError, requireJson, requireSameOrigin, requireUserId } from "../../../lib/finance/security";
...
function errorResponse(error: unknown) {
  if (error instanceof FinanceHttpError) return financeErrorResponse(error);
...
export async function GET(request: Request) {
  try {
    ...
    const rows = await getDb().select().from(calendarEvents).where(eq(calendarEvents.userId, userId)).orderBy(asc(calendarEvents.startsAt));
...
export async function POST(request: Request) {
  try {
    ...
    const row = { id: payload.id || crypto.randomUUID(), userId, ...clean(payload), createdAt: now, updatedAt: now };
...
export async function PUT(request: Request) {
  try {
    ...
    await getDb().update(calendarEvents).set(row).where(and(eq(calendarEvents.id, payload.id), eq(calendarEvents.userId, userId)));
